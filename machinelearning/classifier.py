...
data = pd.read_csv('combined_patterns.csv', encoding='ISO-8859-1', header = None, names = ['Pattern', 'Description'])
...
def predict_design_pattern(description, top_n = 3):
    ...
    top_patterns = [(model.classes_[i], probabilities[i]) for i in top_indicies]
    return top_patterns
